...
def getDatetimeFromTimestamp(timestamp):
    ...
    return datetime(times[0], times[1], times[2], hour=times[3], minute=times[4])
...
def isDesiredLater(minus15, minus10, minus5, current, deltas):
    ...
    # False otherwise
    if (current[0] - minus15[0] != deltas[15] or
        current[0] - minus10[0] != deltas[10] or
        current[0] - minus5[0] != deltas[5]):
        return False
    return True
...
def getSplitData(features, labels, dataSplit):
    ...
    """
    xTrain, xTest, yTrain, yTest = train_test_split(features, labels, train_size=dataSplit, stratify=labels, random_state=SEED)
...
def readData(inputPath):
    ...
    samples = []
    placeholderTimestamp = date(1970, 1, 1) # We only need year, month, and day
    placeholderPeak = (-1, time(1)) # Track the peak amount and the time it occurs in
...
def getSamples1(samples, peaks):
    ...
    comed = []
    comedLabels = []
    deltas = {5 : timedelta(minutes=5), 10 : timedelta(minutes=10), 15 : timedelta(minutes=15), 30 : timedelta(minutes=30)}
...
def getFeatures2(samples, current, i, grid):
    ...
    # Add load for x minutes prior
    minus = getEarlierTime(samples, i, timedelta(minutes=5))
    if minus == None:
        return None
    roc01 = minus[1] if grid == "pjm" else minus[2]
    ...
    if roc4 == None:
        return None
    # Add a rate of change in rate of change over the last 60 minutes, 10 minutes ago
...
def getSamples2(samples, peaks):
    ...
    while i < len(samples):
        ...
        if pjmFeatures == None or comedFeatures == None:
            i += 1
            continue
...
def getBestParameters(estimator, parameters, scoring, trainingData):
    if estimator == "dtree":
        search = GridSearchCV(DecisionTreeClassifier(), parameters, scoring=scoring, error_score="raise")
    elif estimator == "svc":
        search = GridSearchCV(SVC(), parameters, scoring=scoring, error_score="raise")
    else:
...
def getModelScore(estimator, parameters, scoring, trainingData, testingData):
    if estimator == "dtree":
        model = DecisionTreeClassifier(max_depth=parameters["max_depth"], min_samples_split=parameters["min_samples_split"], min_samples_leaf=parameters["min_samples_leaf"], max_features=parameters["max_features"], random_state=SEED, min_impurity_decrease=parameters["min_impurity_decrease"])
    elif estimator == "svc":
        model = SVC(C=parameters["C"], kernel=parameters["kernel"], gamma=parameters["gamma"], random_state=SEED)
    else:
        raise ValueError("Invalid estimator passed to function")
...
def runDtreeTests(pjmTrain, pjmTest, comedTrain, comedTest):
    # A max_depth too large will overfit, but a max_depth too small wil underfit
    maxDepth = [5, 15, 20, 25]
    # Ideal min_split is between 1-40 for sklearn CART implementations of decision trees,
    # so we look in that range. min_split controls overfitting, but too high will
    # cause it to underfit
    minSplit = [2, 5, 10, 20, 30, 40]
    # Ideal min_leaf is between 1-20 and similarly controls overfitting by restricting
    # the tree from making super specific branches for a single sample (or more)
    ...
    # minImpurity generally should be largest when you have a lot of evenly split data,
    # and 0 when all data belongs to the same class. We'll keep it small because
    # most data have the same label, 0
    minImpurity = [0.0, 0.01]
    dtreeParameters = {"max_depth":maxDepth, "min_samples_split":minSplit, "min_samples_leaf":minLeaf, "max_features":maxFeatures, "random_state":[SEED], "min_impurity_decrease":minImpurity}
    
    # Test for prioritizing recall
    pjmBestParam = getBestParameters("dtree", dtreeParameters, "recall", pjmTrain)
    pjmScore = getModelScore("dtree", pjmBestParam, "recall", pjmTrain, pjmTest)
    comedBestParam = getBestParameters("dtree", dtreeParameters, "recall", comedTrain)
...
def runSvcTests(pjmTrain, pjmTest, comedTrain, comedTest, standardize):
    ...
    comedBestParam = getBestParameters("svc", svcParameters, "recall", comedTrain)
    comedScore = getModelScore("svc", comedBestParam, "recall", comedTrain, comedTest)
    results["svc"] = [[pjmBestParam, pjmScore], [comedBestParam, comedScore]]

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Train and compare the performances \
    ...
    pjmTrain, pjmTest, comedTrain, comedTest = preprocess(args.input, args.dataSplit, args.mode)
    if args.dtree:
    ...
    if args.dtree:
        ...
        print("Recall: " + str(pjmBest[1][0]) + "\tPrecision: " + str(pjmBest[1][1]))
